...
function extractDeploymentName(path) {
    const match = path.match(/\/deployments\/([^\/]+)/);
    return match ? match[1] : 'gpt-4o';
}
// Generate mock response
function generateAzureResponse(request, deployment) {
...
function* generateAzureStreamingChunks(request, deployment) {
...
app.post('/openai/deployments/:deploymentName/chat/completions', (req, res) => {
...
app.post('/openai/v1/chat/completions', (req, res) => {
    ...
    console.log(`  Model from request: ${request.model}`);
    if (request.stream) {
        res.setHeader('Content-Type', 'text/plain; charset=utf-8');
        res.setHeader('Cache-Control', 'no-cache');
        res.setHeader('Connection', 'keep-alive');
...
app.get('/openai/deployments', (req, res) => {
    const apiVersion = req.query['api-version'] || '2024-10-21';
...
app.get('/openai/deployments/:deploymentName', (req, res) => {
    const deployment = req.params.deploymentName;
    const apiVersion = req.query['api-version'] || '2024-10-21';
    console.log(`[Mock Azure OpenAI] Get deployment: ${deployment} (API version: ${apiVersion})`);
    const deploymentInfo = MOCK_DEPLOYMENTS[deployment];
    if (deploymentInfo) {
...
export function startMockAzureOpenAI(port = 3002) {
